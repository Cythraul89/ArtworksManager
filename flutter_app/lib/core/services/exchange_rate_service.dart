...
/// Rates are cached to the system temp dir for up to 24 hours; stale cache is
/// returned on network failure so the dashboard stays usable offline.
///
/// Pass [dio] and [apiBase] in tests to point at a local HttpServer.
class ExchangeRateService {
  static const _defaultApiBase = 'https://api.frankfurter.app';

  final Dio _dio;
  final String _apiBase;

  ExchangeRateService({Dio? dio, String? apiBase})
      : _dio = dio ??
            Dio(BaseOptions(
  ...
  Future<File> _cacheFile(String base) async {
    final dir = await getTemporaryDirectory();
    return File(p.join(dir.path, 'rates_$base.json'));
  }

  Future<Map<String, double>?> _readCache(String base) async {
    try {
  ...
  Future<void> _writeCache(String base, Map<String, double> rates) async {
    try {
      final file = await _cacheFile(base);
  ...
  /// Last-modified time of the on-disk cache for [base], or null if absent.
  /// Used by the dashboard to show a stale-rates hint.
  Future<DateTime?> cacheModifiedTime(String base) async {
    try {
      final file = await _cacheFile(base);
  ...
  /// Returns a map of currency code → units per 1 [base] unit.
  ...
  Future<Map<String, double>?> fetchRates(String base) async {
    try {
      ...
      await AppLogger.error(
          'ExchangeRateService: fetch failed for $base, falling back to cache', e, st);
      return await _readCache(base);
    }
  }
}
