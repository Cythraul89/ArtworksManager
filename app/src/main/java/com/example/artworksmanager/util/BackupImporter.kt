 ...
 *
 * The caller is expected to pass the result to [ArtworkRepository.replaceAll], which calls
 * [ArtworkDao.deleteAll] before inserting — so existing records are always cleared first and
 * original IDs in the backup cannot collide with live data.
 */
class BackupImporter(private val context: Context) {
    ...
    fun importFrom(uri: Uri): BackupData {
        ...
        val photosDirCanonical = photosDir.canonicalPath + File.separator
                ...
                while (entry != null) {
                    when {
                        ...
                        entry.name.startsWith("photos/") && !entry.isDirectory -> {
                            ...
                            // ZIP slip protection: verify destination stays inside photosDir
                            if (dest.canonicalPath.startsWith(photosDirCanonical)) {
                                FileOutputStream(dest).use { zis.copyTo(it) }
                                extractedFiles[filename] = dest
                            }
                        }
                    }
                    zis.closeEntry()
        ...
        return parseData(json, extractedFiles)
    }

    private fun parseData(json: String, files: Map<String, File>): BackupData {
        ...
        for (i in 0 until array.length()) {
            ...
            artworks.add(Artwork(
                ...
                photoPath       = o.optString("photo", "").let { files[it]?.absolutePath ?: "" },
                certificatePath = o.optString("certificate", "").let { files[it]?.absolutePath ?: "" },
            ...
            if (o.has("additionalPhotos")) {
                ...
                for (j in 0 until photosArr.length()) {
                    ...
                    val path = p.optString("photo", "").let { files[it]?.absolutePath ?: "" }
                    if (path.isNotEmpty()) {
